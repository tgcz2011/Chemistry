...
1. **物质存在性判定**：输入任意化学式，毫秒级判断“稳定存在 / 仅特定条件下存在 / 可生成但极不稳定 / 通常不存在”，并给出关键注意事项（如氢氧化银只在特定条件下存在、空气中易氧化为氧化银等）。
2. **化学方程式配平**：输入完整方程式（如 `KMnO4+HCl=KCl+MnCl2+H2O+Cl2`）用**代数法**自动配平（保证原子守恒，可处理氧化还原）。
3. **反应补全 + 化学计量**：仅给反应物（如 `HCl+NaOH`，条件可留空）→ 先走本地规则、再由 **Cloudflare Workers AI（Qwen3-30B）** 补全产物并配平，返回完整方程式（`HCl + NaOH → NaCl + H₂O`），并给出各物质摩尔质量与“给某物质的量 → 算其余”的化学计量计算。
